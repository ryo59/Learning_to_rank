...
import torch
import os
from config.dataset_trans import L2RDataset
...
if __name__ == '__main__':
    ...
    for fold in k_fold:
        ...
        model_file = {}
        writer = SummaryWriter(log_dir="./logs/tensorboard_logs_")

        # Path to dataset
        train_file = os.path.join("/MQ2007", fold, "train.txt")
        vali_file = os.path.join("/MQ2007", fold, "vali.txt")
        test_file = os.path.join("/MQ2007", fold, "test.txt")
        ...
        for epoch in range(epochs):
            ...
            for k in [1,3,5,10]:
                ...
                if epoch_valid_ndcg[k] > best_ndcg[k]:
                    ...
                    model_file[k] = '%s-%s-epoch%03d-NDCG@%s：%.5f.pth' % (t1, fold, epoch, k, best_ndcg[k])
        ...
        keika = finish_time - start_time

    with open("./result/result_.txt", "w") as f:
        f.write("Model Architecture：\n{}\nEpoch：{}\n経過時間：{}\n\n".format(model_write, epochs, keika))
